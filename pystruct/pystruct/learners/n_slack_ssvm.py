...
class NSlackSSVM(BaseSSVM):
    ...
    * Joachims, Thorsten and Finley, Thomas and Yu, Chun-Nam John:
    ...
    def _solve_n_slack_qp(self, constraints, n_samples):
        C = self.C
        # constraints = [ [ y_hat, delta_Psi, loss(y,y_hat) ], ... ]
        joint_features = [c[1] for sample in constraints for c in sample]
        losses = [c[2] for sample in constraints for c in sample]

        # this is actually the delta_Psi matrix
        joint_feature_matrix = np.vstack(joint_features).astype(np.float)
        n_constraints = len(joint_features)    # the length of alpha ( x in cvxopt )
        P = cvxopt.matrix(np.dot(joint_feature_matrix, joint_feature_matrix.T)) # ok
        # q contains loss from margin-rescaling
        q = cvxopt.matrix(-np.array(losses, dtype=np.float)) # ok

        # constraints are a bit tricky. 
        # constraint 1: all alpha must be >=zero
        idy = np.identity(n_constraints) 
        tmp1 = np.zeros(n_constraints)

        # constraint 2: box constraint: sum of all alpha for one example must be <= C
        # block matrix where each block consists of 1s to sum up all alpha_iy: \sum_{y \neq y_i} alpha_{i,y} <= C
        ...
        # constraint 3 (optional): positivity constraints:
        ...
        # put together the constraints: Gx <= h
        # 1: -idy * alpha <= 0
        # 2: \sum alpha <= C
        # 3: w >= 0 (optional)
        ...
        if solution['status'] != "optimal":        
            ...
            P = cvxopt.matrix(np.dot(joint_feature_matrix, joint_feature_matrix.T)
                              + 1e-6 * np.eye(joint_feature_matrix.shape[0]))
            solution = cvxopt.solvers.qp(P, q, G, h)
            if solution['status'] != "optimal":
                if solution['status'] == 'unknown':
                    print 'primal objective', solution['primal objective']
                    print 'dual objective', solution['dual objective']
                    print 'gap', solution['gap']
                    print 'relative gap', solution['relative gap']
                    print 'primal infeasibility', solution['primal infeasibility']
                    print 'dual infeasibility', solution['dual infeasibility']
                raise ValueError("QP solver failed. Try regularizing your QP or adjust refinement parameter of solver.")
        ...
        # due to numerical issues, the weights can get smaller than zero even if 
        # we constraint them to be positive; set small values to 0
        self.w[np.where(np.abs(self.w) < 1e-12)]=0.        
