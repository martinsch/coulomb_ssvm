...
class OneSlackSSVM(BaseSSVM):
    ...
    * Joachims, Thorsten and Finley, Thomas and Yu, Chun-Nam John:
    ...
    def _solve_1_slack_qp(self, constraints, n_samples):
        ...
        # cf. Joachims et al, Cutting-Plane Training of Structural SVMs, 2009, pg. 38, opt. problem 7
        ###########
        ...
        try:
            # min 1/2 x' P x + q' x     s.t.   Gx <= h, Ax = b
        ...
        if self.negativity_constraint != None:
            print 'w=', self.w
            print 'w[neg_constr]=', self.w[self.negativity_constraint] 
            assert np.all(self.w[self.negativity_constraint] < 0)
