...
class StructuredModel(object):
    ...
    def inference(self, x, w, relaxed=None, idx=None):
        raise NotImplementedError()

    def batch_inference(self, X, w, relaxed=None):
        # default implementation of batch inference
        return [self.inference(x, w, relaxed=relaxed, idx=idx)
    ...
    def loss(self, y, y_hat):
        ...
        if hasattr(self, 'class_weight'):            
            return np.sum(np.multiply(self.class_weight[y], (y != y_hat))) # force element-wise multiplication
